    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid-cols-2">
        <div className="card">
          ...
          <form onSubmit={handleSave}>
            <div className="input-group">
              ...
              <input type="text" className="input-field" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required placeholder={t('registration.fullNamePlaceholder')} />
            </div>
            <div className="input-group">
              <label className="input-label">{t('registration.phone')}</label>
              <input type="tel" className="input-field" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} required placeholder="+91" />
            </div>
            <div className="input-group">
            ...
            <div className="flex items-center gap-2 mb-4 mt-6" style={{ color: 'var(--primary)' }}>
              <Map />
            ...
            <div className="input-group">
              ...
              <input type="number" step="0.1" className="input-field" value={formData.landSize} onChange={(e) => setFormData({ ...formData, landSize: e.target.value })} required placeholder={t('registration.landPlaceholder')} />
            </div>
            <div className="input-group mb-6">
              <label className="input-label">{t('registration.primaryCrop')}</label>
              <select className="input-field" value={formData.primaryCrop} onChange={(e) => setFormData({ ...formData, primaryCrop: e.target.value })} required>
        ...
        <div className="card">
          ...
          <div style={{ textAlign: 'center', padding: '2rem 1rem' }}>
            <div style={{ width: '120px', height: '120px', borderRadius: '50%', border: '8px solid var(--border)', borderTopColor: 'var(--border)', margin: '0 auto 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--text-muted)' }}>1x</span>
            </div>
            <h4 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--text-main)', marginBottom: '0.5rem' }}>
              {t('registration.onlyOnce')}
            </h4>
